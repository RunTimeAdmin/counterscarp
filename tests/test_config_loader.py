...
class TestTOMLParsing:
    ...
    def test_load_valid_config(self, tmp_path, sample_counterscarp_toml):
        """Test loading a valid TOML config file."""
        config_file = tmp_path / "counterscarp.toml"
        config_file.write_text(sample_counterscarp_toml)
        
        config = load_config(str(config_file))
        
        assert isinstance(config, CounterscarpConfig)
        assert config.engine.name == "Counterscarp Security Engine"
        assert config.engine.version == "3.4.0"
        assert config.engine.fail_on_severity == "HIGH"
        assert config.engine.max_findings == 100

    def test_load_config_with_heuristics(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_config_with_suppressions(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_config_with_static_analysis(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_config_with_fuzzing(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_config_with_reporting(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_config_with_ci(self, tmp_path, sample_counterscarp_toml):
    ...
    def test_load_nonexistent_config_returns_defaults(self):
        ...
        config = load_config("/nonexistent/path/counterscarp.toml")
...
class TestProfileLoading:
    ...
    def test_audit_profile(self, tmp_path, audit_profile_toml):
        ...
        config_file = tmp_path / "counterscarp-audit.toml"
    ...
    def test_pr_profile(self, tmp_path, pr_profile_toml):
        ...
        config_file = tmp_path / "counterscarp-pr.toml"
    ...
    def test_bounty_profile(self, tmp_path, bounty_profile_toml):
        ...
        config_file = tmp_path / "counterscarp-bounty.toml"
...
""")
        ...
        # Should not crash, returns default config or raises CounterscarpConfigError
        try:
            config = load_config(str(config_file))
            assert isinstance(config, CounterscarpConfig)
...
class TestCounterscarpConfig:
    """Test CounterscarpConfig class."""
    ...
    def test_is_finding_suppressed_with_match(self):
        ...
        config = CounterscarpConfig(suppressions=[suppression])
