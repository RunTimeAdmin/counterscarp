...
Sentinel Engine detects vulnerabilities through two complementary rule sets:
...
The following table shows typical bounty ranges for high-value vulnerability patterns that Sentinel Engine detects. Based on Immunefi and Code4rena payout data.
...
Slither detectors are organized by category. Sentinel Engine filters Slither results based on the `[static_analysis.slither]` and `[red_team]` config sections.
...
Configure Slither filtering in `sentinel.toml`:
