...
Sentinel Engine is configured via a `sentinel.toml` file in TOML format. The configuration controls all aspects of the analysis pipeline: which rules are active, what severity thresholds apply, how reports are generated, and which external tools are used.
...
If `--config` is not specified, Sentinel Engine searches for `sentinel.toml` in the current directory and up to 5 parent directories. If no file is found, built-in defaults are used.
...
| `rag_index_path` | string | `".sentinel/rag_index.json"` | Path to RAG vector index |
...
**Note:** The `local` embedding backend uses `sentence-transformers` and works offline with no API keys required. Install with `pip install "sentinel-engine[ai]"`.
...
| `dirs` | list | `[".sentinel/plugins"]` | Directories to scan for plugin modules |

```toml
[plugins]
enabled = true
dirs = [".sentinel/plugins", "/opt/sentinel-plugins"]
```
