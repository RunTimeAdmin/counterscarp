...
try:
    ...
    from exceptions import SentinelConfigError, SentinelValidationError
...
def save_fingerprint_db(fingerprints: List[ProtocolFingerprint], path: str) -> None:
    ...
    Raises:
        SentinelConfigError: If file cannot be written.
    """
    try:
        ...
        if SentinelConfigError:
            raise SentinelConfigError(error_msg, details={"path": path, "error": str(e)})
        raise
...
def load_fingerprint_db(path: str) -> List[ProtocolFingerprint]:
    ...
    Raises:
        SentinelConfigError: If file cannot be read or parsed.
    """
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
